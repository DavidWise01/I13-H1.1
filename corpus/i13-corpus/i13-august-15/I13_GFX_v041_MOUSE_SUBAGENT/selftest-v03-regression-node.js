...
const CASES=[
["call",`def add(I a,I b){ -> a+b }\nI x <- add(2,3)\n`,{x:5}],
["nested",`def inc(I x){ -> x+1 }\ndef twice(I x){ -> inc(inc(x)) }\nI y <- twice(7)\n`,{y:9}],
...
(async()=>{
 ...
 for(const [name,src,expect] of CASES){try{const h=await R.read(src),c=I13WasmVM4.compile(h.ast),v=I13WasmVM4.validate(W,c),r=I13WasmVM4.execute(W,c),ok=v.ok&&r.ok&&Object.entries(expect).every(([k,x])=>r.globals[k]===x);rows.push({name,pass:ok,detail:`calls=${r.calls} depth=${r.max_call_depth}`})}catch(e){rows.push({name,pass:false,detail:e.message})}}
 const pass=rows.every(x=>x.pass);console.log(JSON.stringify({pass,passed:rows.filter(x=>x.pass).length,total:rows.length,tests:rows},null,2));if(!pass)process.exit(1);
