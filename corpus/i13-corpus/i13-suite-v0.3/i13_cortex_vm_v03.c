...
int32_t i13_vm_exec_program(
    ...
    g_vm_steps=0;g_vm_peak=0;g_vm_error_pc=-1;g_vm_error_code=0;g_vm_final_height=0;g_vm_max_call_depth=0;g_vm_calls=0;g_vm_last_func=-1;g_trace_count=0;
            ...
            uint32_t argc=(uint32_t)in.a;if(f_sp[depth]<(int32_t)(argc+1))err=VM_ERR_STACK_UNDERFLOW;
                ...
                if(argc>MAX_LOCALS)err=VM_ERR_LOCAL_OVERFLOW;
                    ...
                    if(tag!=TAG_FUNC)err=VM_ERR_TYPE;
                        ...
                        int fid=(int)v;
                        if(fid<0||(uint32_t)fid>=func_count)err=VM_ERR_BAD_FUNC;
