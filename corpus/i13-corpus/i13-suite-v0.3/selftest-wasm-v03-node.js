...
const R=require("/mnt/data/i13_toolchain_n_v02/i-reader-engine-v0.2.js");global.IReader=R;
...
const I13WasmVM3 = (() => {
  ...
  function execute(W,c,stepLimit=1000000,callLimit=48){
    ...
    const trace=[];
    for(let i=0;i<W.i13_trace_count();i++){
      const fid=W.i13_trace_func(i);
      trace.push({event:TRACE[W.i13_trace_event(i)]||W.i13_trace_event(i),depth:W.i13_trace_depth(i),func_id:fid,func:fid>=0?c.functions[fid]?.name:"main",pc:W.i13_trace_pc(i)});
    }
...
const bytes=fs.readFileSync("./i13_cortex_vm_v03.wasm");
const CASES=[
...
(async()=>{
 const {instance}=await WebAssembly.instantiate(bytes,{}),W=instance.exports,tests=[];
 const t=(name,pass,detail)=>tests.push({name,pass:!!pass,detail});
 for(const [name,src,expect] of CASES){
  const h=await R.read(src,name+".i13"),c=I13WasmVM3.compile(h.ast),v=I13WasmVM3.validate(W,c),r=I13WasmVM3.execute(W,c);
  const ok=v.ok&&r.ok&&Object.entries(expect).every(([k,v])=>r.globals[k]===v);
