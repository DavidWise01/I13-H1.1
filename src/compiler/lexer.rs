...
pub fn lex(source: &SourceFile) -> Result<Vec<Token>, Vec<Diagnostic>> {
    ...
    while i < bytes.len() {
        ...
        match b {
            ...
            b'&' => single(&mut out, TokenKind::Amp, i, line, col, &mut i, &mut col),
            b'|' => single(&mut out, TokenKind::Pipe, i, line, col, &mut i, &mut col),
            b'^' => single(&mut out, TokenKind::Caret, i, line, col, &mut i, &mut col),
            ...
            b'<' if i + 1 < bytes.len() && bytes[i + 1] == b'<' => pair(&mut out, TokenKind::Shl, i, line, col, &mut i, &mut col),
            b'>' if i + 1 < bytes.len() && bytes[i + 1] == b'>' => pair(&mut out, TokenKind::Shr, i, line, col, &mut i, &mut col),
